...
Set<String> defaultStarterDocIds = starterPackages.map((p) => p.docId).toSet();
Set<String> unlockedPackageIds = Set.from(defaultStarterDocIds);
...
bool isPackageCompleted(PackageInfo pkg) {
  bool hasInAllWords = allWords.any((w) => w.packageId == pkg.docId);
  bool hasInCompleted = completedWords.any((w) => w.packageId == pkg.docId);
  return !hasInAllWords && hasInCompleted;
}

bool isPackageInProgress(PackageInfo pkg) {
  if (isPackageCompleted(pkg)) return false;
  return allWords.any((w) => w.packageId == pkg.docId);
}

PackageInfo? get nextPackageToUnlock {
  if (priorityLevel != null) {
    final withinLevel = packagesForLevel(priorityLevel!)
        .where((p) => !isPackageCompleted(p) && !isPackageInProgress(p))
        .toList();
  ...
  for (final p in packageOrder) {
    if (!isPackageCompleted(p) && !isPackageInProgress(p)) return p;
  }
  return null;
}

bool isPackageUnlocked(PackageInfo p) => hasAllPackages || unlockedPackageIds.contains(p.docId);

bool isLevelStarted(String level) => packagesForLevel(level).any((p) => isPackageUnlocked(p) || isPackageInProgress(p));

Future<void> refillWordsIfNeeded() async {
  while (allWords.length < 50) {
    final nextPkg = nextPackageToUnlock;
    if (nextPkg == null) break;

    if (!hasAllPackages && !unlockedPackageIds.contains(nextPkg.docId)) {
      break;
    }

    List<Word> packageWords = [];
    if (packageCache.containsKey(nextPkg.docId)) {
      packageWords = packageCache[nextPkg.docId]!;
    } else {
      try {
        final doc = await FirebaseFirestore.instance.collection('words').doc(nextPkg.docId).get();
        if (doc.exists) {
          final wordsData = doc.data()!['wordsList'] as List;
          packageWords = wordsData.map((w) => Word.fromPackageEntry(w, nextPkg.docId)).toList();
          packageCache[nextPkg.docId] = packageWords.map((w) => w.copy()).toList();
        }
      } catch (e) {
    ...
    if (packageWords.isEmpty) break;

    unlockedPackageIds.add(nextPkg.docId);
    
    int needed = 50 - allWords.length;
    int takeCount = packageWords.length < needed ? packageWords.length : needed;

    for (int i = 0; i < takeCount; i++) {
      allWords.add(packageWords[i].copy());
    }

    if (priorityLevel != null && isLevelCompleted(priorityLevel!)) {
      priorityLevel = null;
    }
  }
  await saveUserData();
...
Future<void> loadUserData() async {
  ...
  if (doc.exists) {
    ...
    unlockedPackageIds.addAll(defaultStarterDocIds);
    if (hasAllPackages) {
...
Future<void> resetUserData() async {
  ...
  unlockedPackageIds = Set.from(defaultStarterDocIds);
  if (hasAllPackages) {
    unlockedPackageIds.addAll(packageOrder.map((p) => p.docId));
  }
...
String? packageStatusLabel(PackageInfo pkg) {
  if (isPackageCompleted(pkg)) return '완료함';
  if (isPackageInProgress(pkg)) return '진행중';
  return null;
}

String? levelStatusLabel(String level) {
  if (isLevelCompleted(level)) return '완료함';
  if (isLevelInProgress(level)) return '진행중';
  return null;
}
    ...
    return Scaffold(
      body: Center(
        child: FadeTransition(
          opacity: _animation,
          child: const Text('초효율 일본어', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  bool _checkCanStartQuiz() {
    if (!hasAllPackages) {
      final nextPkg = nextPackageToUnlock;
      if (nextPkg != null && !unlockedPackageIds.contains(nextPkg.docId)) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              '시험을 보기 위해서는 단어장 ${requiredPackage.displayName} 잠금 해제가 필요합니다.',
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('취소', style: TextStyle(fontSize: 16, color: Colors.grey))),
        ],
      ),
    );
  }
  ...
  void _processAllPackagesPayment() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('전체 패키지 결제가 완료되었습니다! 모든 단어가 해제되었습니다.')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('전체 패키지를 불러오는 중 오류가 발생했습니다.')));
      }
    } finally {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            tooltip: '다크모드',
            icon: Icon(isDarkMode ? Icons.dark_mode : Icons.light_mode, color: primaryTextColor(context)),
            onPressed: () {
              setState(() {
                isDarkMode = !isDarkMode;
                themeModeNotifier.value = isDarkMode ? ThemeMode.dark : ThemeMode.light;
            ...
            child: Center(
              child: SingleChildScrollView(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text('초효율 일본어', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: primaryTextColor(context))),
                    ...
                    ...levelOrder.map((level) {
                      final bool started = isLevelStarted(level);
                    ...
                    const SizedBox(height: 12),
                    Text('현재 ${completedWords.length}개의 단어를 마스터하셨습니다!', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: secondaryTextColor(context))),
                  ],
                ),
              ),
            ),
...
class _QuizScreenState extends State<QuizScreen> with SingleTickerProviderStateMixin {
            ...
            child: Column(
              ...
              children: [
                ...
                RadioListTile<String>(
                  ...
                  value: 'both',
                  groupValue: quizDisplayField,
                  onChanged: (v) => setDialogState(() => quizDisplayField = v!),
                ),
                RadioListTile<String>(
                  contentPadding: EdgeInsets.zero,
    ...
    if (val.trim() == currentWord.jp) {
      ...
      if (_isFirstTry) {
        if (widget.quizType != 'retry') {
          ...
          // 기존에 있던 currentWord.correctCount = 0; 구문을 제거하여 기존 카운트가 유지되도록 함
        }
      ...
      if (nextPkg != null && !unlockedPackageIds.contains(nextPkg.docId)) {
        showDialog(
          ...
          builder: (dialogContext) => AlertDialog(
            title: const Text('잠금 해제 필요', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold)),
            content: Text(
              '시험을 보기 위해서는 단어장 ${nextPkg.displayName} 잠금 해제가 필요합니다.',
              textAlign: TextAlign.center,
              style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500),
            ),
            actions: [
  ...
  Widget build(BuildContext context) {
      ...
      if (_currentSessionIncorrect.isNotEmpty) {
        ...
        resultChildren.add(
          ElevatedButton(
            ...
            onPressed: () async {
              await refillWordsIfNeeded();
              if (!_checkNextQuizAllowed()) return;
    ...
    bool wantKanji = quizDisplayField != 'meaningOnly' && currentWord.kanji.isNotEmpty;
    bool wantMeaning = quizDisplayField != 'kanjiOnly' || currentWord.kanji.isEmpty;
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Text('${_currentIndex + 1} / ${_currentQuizData.length}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                      ),
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              if (inProgress) {
                cardBgColor = isDarkMode ? const Color(0xFF2C3E75) : const Color(0xFFFFF8E7);
              }
...
class _PackageDetailScreenState extends State<PackageDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      if (packageCache.containsKey(widget.package.docId)) {
        setState(() { _words = packageCache[widget.package.docId]; });
        return;
      }
      ...
      if (doc.exists) {
        ...
        packageCache[widget.package.docId] = words.map((w) => w.copy()).toList();
